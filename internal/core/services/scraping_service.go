...
type ScrapingService struct {
	...
	scraperRepository ports.IStoreScrapers
	parsedRepository  ports.IStoreScrapedItems
	contentProvider   ports.IRetrieveWebsiteContent
}

func NewScrapingService(scheduler ports.IScheduleJobs, scraperRepository ports.IStoreScrapers, contentProvider ports.IRetrieveWebsiteContent, parsedRepository ports.IStoreScrapedItems, logger *slog.Logger) *ScrapingService {
		...
		_, err := ps.scheduler.ScheduleJob(scraper.Cron, func() {
			...
			for urlToScrap, scrap := range scraper.Urls {
				...
				for {
					...
					items, err := ps.contentProvider.Retrieve(pagedUrl, scraper.Scraper.ContentSelector)
